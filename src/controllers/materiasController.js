...
// --- DUPLICAR (copia profunda en uno o varios programas) ---
// Clona la materia y TODA su estructura en cada programa destino: temas (modulos),
...
// Clona la materia `origen` (id = materiaId) dentro de `programaDestinoId`.
// Corre dentro de la transacción que abre el llamador: si algo falla, el llamador
// hace ROLLBACK y ese destino queda sin copia (los demás destinos no se afectan).
const clonarMateriaEnPrograma = async (client, { origen, materiaId, businessId, programaDestinoId, nombre }) => {
  // 1. Nueva materia
  ...
  for (const ev of evalRows) {
    const { rows: nuevaEvalRows } = await client.query(
      ...
      [ev.titulo, ev.descripcion, ev.tipo_destino, programaDestinoId, nuevaMateria.id,
  ...
  for (const m of modulos) {
    const { rows: nuevoModRows } = await client.query(
      ...
      [m.titulo, m.descripcion, m.contenido, m.activa, m.orden, programaDestinoId, nuevaMateria.id, businessId]
    );
...
export const duplicarMateria = async (req, res) => {
  const { id } = req.params;
  const businessId = req.user?.bid;
  const { programa_id_destino, programa_ids_destino, nombre } = req.body;

  if (!businessId) {
    return res.status(403).json({ message: 'No se pudo determinar el negocio del usuario.' });
  }

  // Acepta un destino único (compatibilidad) o varios: se hace una copia por programa.
  const crudos = Array.isArray(programa_ids_destino) && programa_ids_destino.length
    ? programa_ids_destino
    : [programa_id_destino];
  ...
  if (!destinos.length) {
    return res.status(400).json({ message: 'Debes indicar al menos un programa destino.' });
  }
  ...
  try {
    ...
    // Programas destino: todos deben existir y pertenecer al mismo negocio
    const { rows: progRows } = await client.query(
      'SELECT id, nombre FROM "public"."programas" WHERE id = ANY($1::int[]) AND business_id = $2',
      [destinos, businessId]
    );
    const progsValidos = new Map(progRows.map((p) => [Number(p.id), p.nombre]));
    const invalidos = destinos.filter((d) => !progsValidos.has(d));
    if (invalidos.length) {
      return res.status(400).json({
        message: 'Alguno de los programas destino no existe o no pertenece a tu negocio.',
      });
    }
    ...
    const creadas = [];
    const fallidas = [];
    for (const programaDestinoId of destinos) {
      try {
        await client.query('BEGIN');
        const nuevaMateria = await clonarMateriaEnPrograma(client, {
          origen, materiaId: id, businessId, programaDestinoId, nombre,
        });
        await client.query('COMMIT');
        creadas.push({ ...nuevaMateria, programa_nombre: progsValidos.get(programaDestinoId) });
      } catch (e) {
        await client.query('ROLLBACK').catch(() => {});
        console.error(`Error al duplicar la materia ${id} en el programa ${programaDestinoId}:`, e);
        fallidas.push({
          programa_id: programaDestinoId,
          programa_nombre: progsValidos.get(programaDestinoId),
        });
      }
    }

    if (!creadas.length) {
      return res.status(500).json({ message: 'Error interno al duplicar la materia.', fallidas });
    }

    return res.status(201).json({
      ok: true,
      materia: creadas[0], // compatibilidad: clientes que esperan una sola materia
      materias: creadas,
      fallidas,
    });
  } catch (error) {

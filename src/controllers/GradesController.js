...
const getGradesByStudentIdController = async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `SELECT g.student_id, g.materia, g.nota 
         FROM grades g
         JOIN students s ON g.student_id = s.id
         WHERE g.student_id = $1 AND s.programa_nombre = $2`,
        [id, 'Validación de bachillerato']
      );
  
      if (result.rows.length === 0) {
        return res.status(404).json({ error: 'No se encontraron notas para este estudiante o el estudiante no pertenece a Validación de Bachillerato' });
      }
